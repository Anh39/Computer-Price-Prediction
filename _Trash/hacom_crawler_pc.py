...
class Handler:
    ...
    async def _get_info(self,url : str):        
        ...
        if (info_table != None):
            ...
            info = self._info_parser2(info_html)
        else:
            info = None
        if (detail_info_table != None):
            detail_info_html = await detail_info_table.inner_html()
            detail_info = self._info_parser(detail_info_html)
        else:
            detail_info = None
    ...
    async def save_spec(self,url,download_directory):
        ...
        if (not os.path.exists(path)):
            try:
                ...
                if ((spec_info['Info'] == None and spec_info['Detail Info']) or spec_info['Price'] == None):
